...
static bool CreateDeviceD3D12(HWND hWnd) {
    ...
    sd.Stereo = FALSE; // Added
    sd.SampleDesc.Count = 1;  // Set to 1 for no multi-sampling (anti-aliasing off)
    sd.SampleDesc.Quality = 0;  // No anti-aliasing quality levels // Added
    sd.BufferUsage = DXGI_USAGE_RENDER_TARGET_OUTPUT;
    sd.Scaling = DXGI_SCALING_NONE; // Added
    sd.SwapEffect = DXGI_SWAP_EFFECT_FLIP_DISCARD;
    sd.AlphaMode = DXGI_ALPHA_MODE_UNSPECIFIED; // Added
    ...
    if (SUCCEEDED(D3D12GetDebugInterface(IID_PPV_ARGS(&debugController)))) {
        ...
        LOG("[!] D3D12 debug layer not available.\n");
    }
#endif
    ...
    desc.Type = D3D12_COMMAND_LIST_TYPE_DIRECT; // Added
    desc.Flags = D3D12_COMMAND_QUEUE_FLAG_NONE; // Added
    ...
    swapChain1->Release(); // not releasing could cause memory leak.
